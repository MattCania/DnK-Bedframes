...
<section class="min-h-screen bg-zinc-900 p-6 text-white">
	...
	<!-- Tabs -->
	<div class="mb-4 flex overflow-x-auto rounded-md border border-zinc-700 bg-zinc-800 text-sm">
		{#each tabs as t}
			<button
	...
	{#if (ordersFor(tab)?.length ?? 0) === 0}
		<p class="text-zinc-300">No orders in this section.</p>
	{:else}
		<div class="space-y-6">
			{#each ordersFor(tab) as order (order.id)}
				<div class="rounded-lg border border-zinc-700 bg-zinc-800 p-4 shadow-sm">
					<div class="mb-3 flex flex-wrap items-center justify-between gap-3">
						...
						<div class="text-right font-semibold text-green-400">
					...
					<div class="space-y-3">
						{#each order.items as item}
							<div
								class="flex items-center justify-between gap-4 rounded-md border border-zinc-700 p-3"
							>
					...
					{#if tab === 'requests'}
						<div class="mt-4 flex justify-end gap-3">
							<form method="POST" action="?/deny" use:enhance={onEnhance}>
								...
								<button
									type="submit"
									class="rounded border border-red-600 px-4 py-2 text-red-400 hover:bg-red-900/20"
									>Deny</button
								>
							</form>
